...
impl ApiBuilder {
    ...
    pub fn build(self) -> Result<Api, ApiError> {
        ...
        let tls_builder = builder()?;
        let agent: Agent = tls_builder.redirect_auth_headers(RedirectAuthHeaders::SameHost).build().into();
...
fn builder() -> Result<ConfigBuilder<AgentScope>, ApiError> {
    // PlatformVerifier disables bundled webpki roots and uses the OS trust
    // store instead (SecureTransport on macOS). This is required behind
    // Zscaler-style MITM proxies whose CA cert lives in the system keychain
    // but is absent from webpki's bundled root list.
